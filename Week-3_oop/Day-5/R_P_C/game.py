...
class Game:
    item_type = ["r", "p", "s"]
    win_combinations = ["rs", "sp", "pr"]

    def get_user_item(self) -> str:
        while True:
            item = input("Select (r)ock, (p)aper or (s)ciccors: ")
            if item in Game.item_type:
                return item
            else:
                print("wrong item. Try again.")

    def get_computer_item(self) -> str:
        return random.choice(Game.item_type)

    def get_game_result(self, user_item, computer_item) -> str:
        if user_item == computer_item:
            return "draw"
        fact_comb = user_item + computer_item
        if fact_comb in Game.win_combinations:
            return "win"
        return "loss"

    def play(self) -> str:
        user_item = Game.get_user_item(self)
        computer_item = Game.get_computer_item(self)
        result = Game.get_game_result(self, user_item, computer_item)
        print(f"You selected: {user_item}. The computer" +
              f"selected: {computer_item}. Result: {result}\n")
        return result
